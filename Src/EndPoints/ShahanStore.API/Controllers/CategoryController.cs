...
public class CategoryController(IMediator mediator, IMapper mapper, ILocalFileService localFileService) : ApiController
{
    ...
    public async Task<IActionResult> ChangeBanner([FromForm] ChangeCategoryBannerDto request, CancellationToken cancellationToken)
    {
        ...
        string bannerImgName = await localFileService.SaveFileAsync(request.BannerImg, AppDirectories.CategoryBanner);
        ...
        {
            localFileService.DeleteFile(result.Data, AppDirectories.CategoryBanner);
        }
        return HandleResult(result);
    }
    ...
    public async Task<IActionResult> ChangeIcon([FromForm] ChangeCategoryIconDto request, CancellationToken cancellationToken)
    {
        ...
        string iconImgName = await localFileService.SaveFileAsync(request.Icon, AppDirectories.CategoryIcon);
        ...
        if (result.Data is not null)
        {
            localFileService.DeleteFile(result.Data, AppDirectories.CategoryIcon);
        }
        return HandleResult(result);
    }
    #endregion
